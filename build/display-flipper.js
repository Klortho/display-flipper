...
class Panel {
  ...
  animateTo(yScale, onEnd) {
    this.g.transition().duration(this.opts.duration / 2).ease(this.easing).attr('transform', 'scale(1, ' + yScale + ')').on('end', onEnd);
    return this;
  }
}
...
class PanelText {
  ...
  setValue(value, rbb) {
    ...
    const text$$1 = g.append('text').attrs({
      x: 0,
      y: 0,
      fill: opts.textColor,
      stroke: 'none',
    ...
    const tbb = text$$1.node().getBoundingClientRect();
    ...
    const scale = tbb.width <= rw & tbb.height <= rh ? 1 : Math.min(rw / tbb.width, rh / tbb.height);

    g.attr('transform', 'scale(' + scale + ') ' + 'translate(' + tx + ', ' + ty + ')');
  ...
  clone(parentG) {
    ...
    ['value', 'tx', 'ty', 'scale'].forEach(prop => newPT[prop] = this[prop]);
  ...
  constructor(parentG, x, y, options) {
    ...
    ['top', 'bottom'].forEach(side => {
      ...
      ['under', 'over'].forEach(depth => {
        const pg = opts.debug ? g.append('g').attr('transform', 'translate(' + 30 * panels.length + ', 0)') : g;
...
class Flipper {
  constructor(selector$$1, digitNames, options = null, x = 0, y = 0) {
    const svg$$1 = select(selector$$1);
    const opts = Object.assign({}, defaults, options);
    const g = svg$$1.append('g');
    const digits = [];
    const byName = {};
    var lastX = null;
    var lastName = null;
    digitNames.forEach(curName => {
      const curX = lastX === null ? x : lastX + opts.width + opts.gap(lastName, curName);
...
//# sourceMappingURL=display-flipper.js.map
